...
public class Boid {    
    ...
    public Vector avoidCollision(Iterable<Boid> neighbors)
    {
        ...
        for (Boid b : neighbors)
        {
            ...
            Vector scaledAvoidanceVector = avoidanceVector.times(1.0 / distanceTo);            
    ...
    {
        Vector avoidanceVector = avoidCollision(neighbors).times(BOID_AVOIDANCE_WEIGHT);
        Vector hawkAvoidanceVector = avoidCollision(hawk).times(HAWK_AVOIDANCE_WEIGHT);
        Vector matchingVector = matchVelocity(neighbors).times(VELOCITY_MATCH_WEIGHT);
        Vector plungingVector = plungeDeeper(neighbors).times(PLUNGE_DEEPER_WEIGHT);
        Vector returnVector = returnToWorld().times(RETURN_TO_ORIGIN_WEIGHT);
        ...
        if (desired.magnitude() == 0.0)
            return desired;
        return desired.direction().times(THRUST_FACTOR);
    }
